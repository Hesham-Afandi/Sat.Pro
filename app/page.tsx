    ...
    <div className="min-h-screen bg-white" dir="rtl">
      ...
      <section className="bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 text-white py-20 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/courses" className="px-8 py-4 bg-white text-indigo-600 rounded-xl font-bold text-lg hover:bg-gray-100 transition shadow-lg">
              ابدأ الآن مجاناً
            </Link>
            <Link href="/exam" className="px-8 py-4 bg-transparent border-2 border-white text-white rounded-xl font-bold text-lg hover:bg-white/10 transition">
              جرب امتحان تجريبي
            </Link>
          </div>
        </div>
      ...
      <section className="py-16 px-4 bg-gray-50">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl font-bold text-center text-gray-900 mb-12">لماذا تختار منصتنا؟</h2>
      ...
      <section className="py-16 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link href="/courses" className="text-indigo-600 font-medium hover:underline">عرض الكل ←</Link>
          </div>
          {courses.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {courses.map((course: any) => (
                <Link key={course.id} href={`/courses/${course.id}`} className="block bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition">
                  <div className="h-40 bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center">
                    <span className="text-white text-4xl font-bold opacity-30">{course.subject?.charAt(0) || "S"}</span>
                  </div>
                  <div className="p-5">
                    ...
                    <p className="text-gray-600 text-sm line-clamp-2">{course.description || "وصف الكورس سيظهر هنا..."}</p>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <section className="py-16 px-4 bg-indigo-600 text-white">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-indigo-100 text-lg mb-8">انضم لآلاف الطلاب الذين حققوا درجات عالية في الـ SAT</p>
          <Link href="/register" className="inline-block px-8 py-4 bg-white text-indigo-600 rounded-xl font-bold text-lg hover:bg-gray-100 transition shadow-lg">
            أنشئ حسابك مجاناً
          </Link>
        </div>
      </section>
