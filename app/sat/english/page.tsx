...
export default function SATEnglishPage() {
  ...
  useEffect(() => {
    async function fetchCourses() {
      try {
        const res = await fetch('/api/courses?subject=English&category=SAT');
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4" dir="rtl">
      <div className="max-w-6xl mx-auto">
        {/* الهيدر */}
        <div className="bg-gradient-to-r from-amber-500 to-amber-600 rounded-3xl p-8 text-white text-center mb-8">
          <div className="text-6xl mb-4">📖</div>
          <h1 className="text-4xl font-bold mb-2">SAT English</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {courses.map((course) => (
              <Link
                ...
                className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition p-6 border-r-4 border-amber-500"
              >
